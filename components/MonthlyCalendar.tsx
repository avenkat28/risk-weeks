...
export function MonthlyCalendar({ assignments, commitments }: { assignments: AssignmentItem[]; commitments: PersonalCommitment[] }) {
  ...
  return <section className="rounded-lg border border-ink/10 bg-white p-5 shadow-sm">
    ...
    <div className="mt-5 grid grid-cols-7 border-l border-t border-ink/10">{["Mon","Tue","Wed","Thu","Fri","Sat","Sun"].map(d=><div key={d} className="border-b border-r border-ink/10 bg-paper p-2 text-center text-xs font-bold text-ink/50">{d}</div>)}
      {cells.map((date,i)=>{const key=dateKey(date); const due=assignments.filter(a=>a.date===key); const busy=commitments.filter(c=>c.date===key); const events=[...due.map(a=>({id:`assignment-${a.id}`,title:a.title,time:a.startTime,kind:"assignment" as const,color:a.courseColor,completed:a.completed})),...busy.map(c=>({id:`commitment-${c.id}`,title:c.title,time:c.startTime,kind:"commitment" as const,color:"",completed:false}))].sort((a,b)=>(a.time??"24:00").localeCompare(b.time??"24:00")); return <div key={i} className={`min-h-24 border-b border-r border-ink/10 p-1.5 ${date.getMonth()!==month.getMonth()?"bg-paper/50 text-ink/30":""}`}><p className="text-xs font-semibold">{date.getDate()}</p><div className="mt-1 space-y-1">{events.slice(0,4).map(event=><div key={event.id} title={`${event.time ? `${event.time} · ` : ""}${event.title}`} className={`truncate rounded px-1.5 py-1 text-[10px] font-semibold ${event.kind === "commitment" ? "border border-amber-300 bg-lemon text-ink" : "text-white"} ${event.completed?"opacity-40 line-through":""}`} style={event.kind === "assignment" ? {backgroundColor:event.color} : undefined}>{event.time ? <span className="mr-1 opacity-75">{event.time}</span> : null}{event.title}</div>)}{events.length>4?<p className="text-[10px] text-ink/50">+{events.length-4} more</p>:null}</div></div>})}
